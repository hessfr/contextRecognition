...
//		Log.i(TAG, updatePoints.toString());
		...
		// Log.d(TAG, n_components_new + " new components");

		ContextClassModel newModel = trainGMM(updatePoints, n_components_new);

		DenseMatrix64F likelihoods = new DenseMatrix64F(n_components_new,
				n_novel);
		...
		for (int i = 0; i < n_components_new; i++) {
			...
			CommonOps.extract(newModel.get_means(), i, (i + 1), 0, n_features,
					tmpMeans, 0, 0);
		...
		/*
		 * The first element in this ArrayList is the matrix of all feature
		 * points that are assigned to the first component (of the novel model),
		 * and so on...
		 */
		ArrayList<DenseMatrix64F> Dk = new ArrayList<DenseMatrix64F>();
		// Mk contains the number of points for each entry (component) in the Dk
		// ArrayList:
		...
		for (int c = 0; c < n_components_new; c++) {
			...
			DenseMatrix64F tmpMatrix = Dk.get(c); // Create temp copy of the
													// matrix
			int idx = 0; // Counts current index in the matrix for the current
							// component

			for (int i = 0; i < n_novel; i++) {
				if (pointComponents.get(0, i) == c) {

					CommonOps.extract(updatePoints, i, (i + 1), 0, n_features,
							tmpMatrix, idx, 0);
		...
		/*
		 * Mapping from OLD to NOVEL (!) components, if old components is not
		 * mapped to novel one, the value equals -1. Elements initialized to -1.
		 * mapping[3] = 5 means that old component 3 is equal to new component
		 * 5.
		 */
		...
		for (int k = 0; k < n_components_new; k++) {
			for (int j = 0; j < n_components_old; j++) {
				...
				if (covarTest(Dk.get(k), oldGMM.clf(label).get_covars()[j]) == true) {
					Log.d(TAG, "Covariance test passed");

					DenseMatrix64F tMeans = new DenseMatrix64F(1, n_features);
					CommonOps.extract(oldGMM.clf(label).get_means(), j,
							(j + 1), 0, n_features, tMeans, 0, 0);

					if (meanTest(Dk.get(k), tMeans) == true) {
						Log.d(TAG, "Mean test passed");
		...
		for (int j = 0; j < n_components_old; j++) {
			...
			if (k != -1) {
				...
				// System.out.println("Merging old component " + j +
				// " with new component " + k);

				DenseMatrix64F tmpMeansOld = new DenseMatrix64F(1, n_features);
				CommonOps.extract(oldGMM.clf(label).get_means(), j, (j + 1), 0,
						n_features, tmpMeansOld, 0, 0);

				DenseMatrix64F tmpMeansNovel = new DenseMatrix64F(1, n_features);
				CommonOps.extract(newModel.get_means(), k, (k + 1), 0,
						n_features, tmpMeansNovel, 0, 0);

				ComponentObject mergedComp = mergeComponents(n_train_old,
						n_novel, Mk.get(k), oldGMM.clf(label).get_weights()
								.get(0, j), newModel.get_weights().get(0, k),
						tmpMeansOld, tmpMeansNovel, oldGMM.clf(label)
								.get_covars()[j], newModel.get_covars()[k]);
				...
				ComponentObject histComp = addHistoricalComponent(n_train_old,
						n_novel, oldGMM.clf(label).get_weights().get(0, j),
		...
		// Loop through the new component to check which ones haven't been
		// merged
		for (int i = 0; i < n_components_new; i++) {
			...
			if (inArray == false) {
				// Add novel component
				DenseMatrix64F tmpMeansNovel = new DenseMatrix64F(1, n_features);
				CommonOps.extract(newModel.get_means(), i, (i + 1), 0,
						n_features, tmpMeansNovel, 0, 0);

				ComponentObject novelComp = addNovelComponent(n_train_old,
						n_novel, Mk.get(i), tmpMeansNovel,
						newModel.get_covars()[i]);
		...
		for (int i = 0; i < componentList.size(); i++) {
			...
			CommonOps.extract(componentList.get(i).get_means(), 0, 1, 0,
					n_features, means, i, 0);
	...
	/*
	 * Calculate W statistic to determine if both components have equal
	 * covariance
	 * 
	 * @param covars_old: old covariance matrix for one component, shape:
	...
	/*
	 * Use Hotelling T-squared test to determine if both components have equal
	 * means
	 * 
	 * @param means_old: old mean values for one component, shape: (1 x
	 * n_features)
	 * 
	 * @return: True if covars are equal, False if not
	 */
	...
	private ComponentObject mergeComponents(int n_old, int n_novel, int n_comp,
			double weight_old, double weights_novel, DenseMatrix64F means_old,
			DenseMatrix64F means_novel, DenseMatrix64F covars_old,
			DenseMatrix64F covars_novel) {
		...
		double c2;
		c2 = ((n_old * weight_old * prodOld) + (n_comp * prodNovel))
				/ ((double) n_old * weight_old + n_comp);
	...
	private ComponentObject addHistoricalComponent(int n_old, int n_novel,
			double weight_old, DenseMatrix64F means_old,
			DenseMatrix64F covars_old) {
	...
	private ComponentObject addNovelComponent(int n_old, int n_novel,
			int n_comp, DenseMatrix64F means_novel, DenseMatrix64F covars_novel) {
	 ...
	 */
	private double BIC(DenseMatrix64F featureData, ContextClassModel model,
			int n_components) throws IOException {

		int n_samples = featureData.numRows;
		int n_features = featureData.numCols;

		DenseMatrix64F lp = computeLogProb(featureData, model.get_weights(),
				model.get_means(), model.get_covars());

		double logLik = -2.0 * CommonOps.elementSum(lp);

		double complexity = ((n_components / 2.0 * (n_features + 1) * (n_features + 2)) - 1.0)
				* Math.log(n_samples);
	...
	/*
	 * Calculate Probability Density Function of Gaussian in multiple
	 * dimensions. Call this function for each component of the mixture
	 * individually
	 * 
	 * @param data: Feature data
	 * 
	 * @param means: Row vector representing the mean values of a single
	 * component
	 * 
	 * @param covar: Covariance matrix of the component
	 */
	private DenseMatrix64F pdf(DenseMatrix64F data, DenseMatrix64F means,
			DenseMatrix64F covar) {
	...
	/*
	 * Train a new mixture model for one class. featureData has to be scaled
	 * already.
	 */
	private ContextClassModel trainGMM(DenseMatrix64F featureData,
			int n_components) throws IOException {
		...
		KMeansClusterer clusterer = new KMeansClusterer(new SimpleMatrix(
				X_train));
		List<KMeansClusterer.Centroid> centroids = clusterer
				.initializeRandom(n_components);
		List<KMeansClusterer.Centroid> clusterCent = clusterer
				.cluster(centroids);
		...
		if (clusterCent.size() != n_components) {
			Log.e(TAG, "Dimensions missmatch: number of clusters not equal to number of components");
		}

		for (int i = 0; i < n_components; i++) {
			DenseMatrix64F tmp = new DenseMatrix64F(clusterCent.get(i)
					.getFeatures().getMatrix());

			// Fill the means matrix with the clusters (each column refers to
			// one components):
		...
		for (int k = 0; k < n_steps; k++) {
			...
			// ----- E-Step -----
			DenseMatrix64F proba = computeLogProb(X_train, weights, means,
					covars);
			...
			for (int i = 0; i < n_samples; i++) {
				...
				CommonOps.extract(lprTmp, i, (i + 1), 0, n_components, tmpRow,
						0, 0);
			...
			DenseMatrix64F diff = new DenseMatrix64F(n_samples, n_components); // =
																				// lprTmp
																				// -
																				// proba
			...
			for (int i = 0; i < n_components; i++) {
				...
				CommonOps.extract(lprTmp, 0, n_samples, i, (i + 1), tmpCol, 0,
						0);
			...
			DenseMatrix64F responsibilities = new DenseMatrix64F(n_samples,
					n_components);
			...
			for (int row = 0; row < n_samples; row++) {
				for (int col = 0; col < n_components; col++) {
					responsibilities
							.set(row, col, Math.exp(diff.get(row, col)));
				}
			}

			// Check for convergence:
			if ((k > 0)
					&& (Math.abs(llList.get(llList.size() - 1)
							- llList.get(llList.size() - 2)) < 0.01)) {
				converged = true;
				// System.out.println("EM converged after " + k +
				// " iterations");
				break;
			}
			...
			DenseMatrix64F weighted_X_sum = new DenseMatrix64F(n_components,
					n_features);
			DenseMatrix64F respTrans = new DenseMatrix64F(n_components,
					n_samples);
			...
			// --- Update means ---
			// multiply each column of weighted_X_sum element wise with
			// inverse_weights:
			for (int i = 0; i < n_features; i++) {
				DenseMatrix64F tmpCol = new DenseMatrix64F(n_components, 1);
				CommonOps.extract(weighted_X_sum, 0, n_components, i, (i + 1),
						tmpCol, 0, 0);
			...
			for (int c = 0; c < n_components; c++) {
				...
				CommonOps.extract(responsibilities, 0, n_samples, c, (c + 1),
						post, 0, 0);
				CommonOps.transpose(post);

				DenseMatrix64F tmp1 = new DenseMatrix64F(n_features, n_samples); // (post
																					// *
																					// tmpTrain.T)
																					// in
																					// Python
				// Element-wise multiply each row of transposed matrix of
				// training points with post vector:
				for (int i = 0; i < n_features; i++) {
					DenseMatrix64F tmpRow = new DenseMatrix64F(1, n_samples);

					CommonOps.extract(transPoints, i, (i + 1), 0, n_samples,
							tmpRow, 0, 0);
				...
				DenseMatrix64F dotProd = new DenseMatrix64F(n_features,
						n_features); // dot product of tmp1 and X_train

				CommonOps.mult(tmp1, X_train, dotProd);

				double postSum = CommonOps.elementSum(post) + 10.0 * EPS;

				DenseMatrix64F avgCV = new DenseMatrix64F(n_features,
						n_features);
				...
				DenseMatrix64F mu_squared = new DenseMatrix64F(n_features,
						n_features);
		...
		if (converged == false) {
			Log.w(TAG, "EM not converged after " + n_steps
					+ " iterations");
		}
	...
	private DenseMatrix64F computeLogProb(DenseMatrix64F X,
		...
		for (int c = 0; c < n_components; c++) {
			...
			// ============ Implementation using Apache Commons to compute
			// Cholesky: =============
			...
			//
			// try {
			// cholesky = new CholeskyDecomposition(rmL0, symetryThreshold,
			// positivityThreshold);
			// } catch
			// (org.apache.commons.math3.linear.NonSymmetricMatrixException e) {
			//
			// System.out.println("Cholesky Decomposition failed for component "
			// + c + ", dumping current covars");
			...
			//
			// RealMatrix minAdd =
			// MatrixUtils.createRealIdentityMatrix(n_features);
			// minAdd.scalarAdd(1e-18);
			//
			// cholesky = new CholeskyDecomposition(minAdd, symetryThreshold,
			// positivityThreshold);
			//
			// // For other exception:
			// // RealMatrix id3 =
			...
			// ============ Implementation using EJML method to compute
			// Cholesky: ================

			DenseMatrix64F L0 = new DenseMatrix64F(n_features, n_features);
			GenericMatrixOps.copy(covars[c], L0);

			CholeskyDecomposition<DenseMatrix64F> chol = DecompositionFactory
					.chol(n_features, true);
			...
			for (int i = 0; i < n_samples; i++) {
				...
				CommonOps.extract(distMeanTransp, 0, n_features, i, (i + 1),
						tmpCol, 0, 0);
		...
		CommonOps.transpose(logProbs); // refers to tmpArray (in Python) at this
										// point
	...
	private void dumpArrayJSON(double[][] arr, String filename)
			throws IOException {
	...
	/*
	 * Create matrix that contains the index of the maximum element for each
	 * column vector. Identical to np.argmax(matrix, 0) in numpy.
	 */
	...
	/*
	 * Converts an ArrayList of double arrays into a EJML DenseMatrix64F
	 */
	private DenseMatrix64F convertToEJML(ArrayList<double[]> in) {
